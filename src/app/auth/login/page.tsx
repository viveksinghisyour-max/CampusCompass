...
import { Compass, Mail, Lock, User, ArrowRight, KeyRound } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Simulate credential identification based on username
      const name = email.split('@')[0];
      const displayName = name.charAt(0).toUpperCase() + name.slice(1);
      
      const success = await login(email, displayName);
      if (success) {
        toast.success(`Welcome back, ${displayName}! You have logged in successfully.`);
        router.push('/dashboard');
      }
    } catch {
  ...
  const handleForgotSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!forgotEmail.trim()) {
      toast.error('Please input your registered email address first.');
      return;
    }
    setForgotLoading(true);
    setTimeout(() => {
      setForgotLoading(false);
      toast.success(`A password recovery and reset link has been dispatched to ${forgotEmail}. Please inspect your inbox.`);
      setForgotMode(false);
      setForgotEmail('');
    }, 1000);
  };
  ...
  return (
    <div className="flex-1 flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8 bg-slate-50/50 relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-6 z-10 text-left">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400 font-semibold leading-relaxed">
            {forgotMode 
              ? 'Key in your email address to recover your password details' 
              : 'Access your saved colleges dashboard and customized comparator boards'}
          </p>
        </div>
          ...
          {!forgotMode ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Button
                type="submit"
                isLoading={loading}
                className="w-full text-sm font-black shadow-lg shadow-primary/10 mt-2"
              >
                Sign In to Account
              </Button>
              ...
              {/* Social connector grid UI only */}
              <div className="grid grid-cols-3 gap-3">
                <button
                  type="button"
                  onClick={() => toast.info('Google Single Sign-On is simulated on frontends.')}
                ...
                <button
                  type="button"
                  onClick={() => toast.info('GitHub Single Sign-On is simulated on frontends.')}
                  className="flex items-center justify-center h-10 border border-slate-200 hover:bg-slate-50 transition-colors rounded-xl cursor-pointer"
                >
                ...
                <button
                  type="button"
                  onClick={() => toast.info('Apple Single Sign-On is simulated on frontends.')}
                  className="flex items-center justify-center h-10 border border-slate-200 hover:bg-slate-50 transition-colors rounded-xl cursor-pointer"
                >
                  <svg className="w-4.5 h-4.5 text-slate-800" viewBox="0 0 24 24" fill="currentColor">
            ...
            <form onSubmit={handleForgotSubmit} className="space-y-5">
              <div className="p-4 rounded-2xl border border-indigo-100 bg-indigo-50/20 text-indigo-800 flex gap-3 text-xs leading-relaxed font-semibold">
                <KeyRound className="w-4.5 h-4.5 text-indigo-500 shrink-0 mt-0.5" />
                <span>Input your registered email. We will forward a secure URL containing instructions to create a new credentials passcode.</span>
              </div>
              ...
              <button
                ...
                className="w-full text-center text-xs font-bold text-slate-500 hover:text-slate-700 hover:underline pt-2 cursor-pointer block"
              >
                Back to Sign In
              </button>
            </form>
        ...
        {!forgotMode && (
          <p className="text-center text-xs font-bold text-slate-400">
            ...
            <Link href="/auth/signup" className="text-primary hover:underline">
              Create a free profile <ArrowRight className="w-3.5 h-3.5 inline ml-0.5" />
            </Link>
          </p>
        )}
